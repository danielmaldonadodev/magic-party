...
function ProfessionalFab({ theme }) {
  return (
    <Link
      ...
      className={`fixed right-4 sm:right-6 z-50 hidden sm:inline-flex items-center gap-2 rounded-full ${theme.gradient} px-4 py-2.5 sm:px-5 sm:py-3 text-white shadow-xl ring-1 ring-black/10 transition-all duration-300 hover:shadow-2xl hover:scale-105 focus:outline-none focus:ring-4 ${theme.colors.ring} animate-float-subtle`}
      style={{ 
...
function CalendarGrid({
  ...
  const chipColor = (ev) => {
    ...
    switch (ev.participation?.status) {
      case 'going': return 'border-green-400 bg-green-100 text-green-800'
      ...
      <Card className="relative overflow-hidden bg-white/90 backdrop-blur-sm border border-gray-200 shadow-lg" padding="none">
        ...
        <div className="px-4 sm:px-6 py-4 border-b border-gray-200">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center gap-2">
              <button onClick={prev} className="p-2 rounded-lg hover:bg-gray-100">◀</button>
              <h2 className="text-lg sm:text-xl font-semibold text-gray-900 min-w-[200px] text-center">
                {format(currentDate, "MMMM 'de' yyyy", { locale: es })}
              </h2>
              <button onClick={next} className="p-2 rounded-lg hover:bg-gray-100">▶</button>
            </div>
            <button onClick={today} className="rounded-lg px-3 py-2 text-sm font-medium border border-gray-300 hover:bg-gray-50">
              Hoy
            </button>
          </div>
        </div>
        ...
        {/* Cabeceras semana */}
        <div className="grid grid-cols-7 px-4 sm:px-6 pt-3 text-xs sm:text-sm font-medium text-gray-600">
          {['Lun','Mar','Mié','Jue','Vie','Sáb','Dom'].map(d => <div key={d} className="py-2 text-center">{d}</div>)}
        </div>
        ...
        <div className="grid grid-cols-7 gap-[1px] bg-gray-200">
          {calendarDays.map((day) => {
            ...
            return (
              <div key={key} className={`min-h-[96px] sm:min-h-[120px] bg-white p-2 sm:p-3 ${inMonth ? '' : 'bg-gray-50'}`}>
                <div className="flex items-center justify-between">
                  <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-medium ${
                    todayFlag ? 'bg-blue-600 text-white' : 'text-gray-700'
                  }`}>
                    {format(day, 'd')}
                  </span>
                </div>
                ...
                <div className="mt-2 space-y-1.5">
                  {dayEvents.slice(0,3).map(ev => {
                    ...
                    return (
                      <button
                        key={ev.id}
                        onClick={() => onShowCalendar?.(ev, profiles.find(p => p.id === ev.created_by)?.nickname || 'Organizador')}
                        className={`w-full text-left rounded-md border px-2 py-1.5 text-[11px] sm:text-xs hover:shadow transition ${chipColor(ev)}`}
                        title={ev.title}
                      >
                        <div className="flex items-center gap-1.5">
                          <span>{status.key === 'past' ? '✅' : status.key === 'active' ? '🔴' : status.key === 'soon' ? '⏰' : '📅'}</span>
                          <span className="truncate font-medium">{ev.title}</span>
                        </div>
                        <div className="mt-0.5 flex items-center gap-1 text-[10px] sm:text-[11px] opacity-80">
                          <span>{format(new Date(ev.starts_at), 'HH:mm')}–{format(new Date(ev.ends_at), 'HH:mm')}</span>
                          {ev.location && <span>· {getLocationIcon(ev.location)} {ev.location}</span>}
                        </div>
                      </button>
                    )
                  })}
                  {dayEvents.length > 3 && (
                    <button
                      onClick={() => onShowCalendar?.(dayEvents[0], profiles.find(p => p.id === dayEvents[0].created_by)?.nickname || 'Organizador')}
                      className="text-[11px] sm:text-xs text-blue-700 hover:underline"
                    >
...
export default function ProfessionalEventsList({
  ...
  if (loading) {
    return (
      <div className="min-h-screen theme-transition pb-20 sm:pb-24" style={{ background: `linear-gradient(135deg, ${theme.backgroundGradient})` }}>
